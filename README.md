# itch.io
Check this link for best gameplay and details.
<p align="center">
  https://lengocnguyenminh.itch.io/noah
</p>

# NOAH
Welcome to the journey, adventurer!!! Cross the roads, kill the monsters, collect all 5 stones to revive the god HAON - the god of protection!!!
<p align="center">
  <img width="705" height="400" alt="image" src="https://github.com/user-attachments/assets/a1ba6f23-f4a0-4493-b5e6-4c0f5f9ab8ff" />
</p>


# Let see some image in Game
<p align="center">
  <img width="794" height="443" alt="image" src="https://github.com/user-attachments/assets/e9633cf2-332d-4b45-898f-c93466f0f8b8" />
  <img width="794" height="396" alt="image" src="https://github.com/user-attachments/assets/2d0f5fc9-021f-44e7-8a58-792e6108782d" />
  <img width="794" height="471" alt="image" src="https://github.com/user-attachments/assets/828ef58f-44c7-4340-8186-eccc1084f96e" />
  <img width="794" height="472" alt="image" src="https://github.com/user-attachments/assets/a25ebb29-c487-48de-ae6e-97016af3827b" />
  <img width="794" height="505" alt="image" src="https://github.com/user-attachments/assets/6b8486dc-335d-4018-bb54-27778fb36ae1" />
</p>

# Game features
- Player level: Player when level up gain points and can spend that point to health, damage or bullet in player status panel.<br>
- Player status panel: Check player information and change attributes.(press C)<br>
- Inventory: Collect items and they will be here. you can also check item information or add they to hot bar.(press I)<br>
- Mission: List of missions you can do for rewards. (press J)<br>
- Pause: pause the game. (press ESC)<br>
- Save: can save and load (press F on checkpoint).<br>
- Player loadout: Player can change hp items and weapons.<br>
- Enemy and Boss: 3 type of normal enemies and 2 boss. The hard level depend on player current level (higher level mean harder).<br>
- Dialogue: Player and speak and choose answer, which lead to different respond. (press F near NPC)<br>
- Shop: Player can buy item at 7 A.M. (press F near shop)<br>
- Environment: All draw by hand with animation, shader.<br>
- Player: Have health system and level system, can shoot and dash.<br>



